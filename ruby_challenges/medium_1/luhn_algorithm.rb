...
# Can check if it is valid per the Luhn formula. This should treat, for example, "2323 2005 766 3554" as valid.
...
#split the number into an array of add able numbers
  #- each second number is the number double, if that value of 2xnum is greater/= 10 subtract 9
...
class Luhn
  ...
  def self.create(invalid_number)
    counter = 0
    loop do
      luhn_obj = Luhn.new(invalid_number)
      luhn_obj.input_number = invalid_number.to_s.chars.map(&:to_i).append(counter).join.to_i
      return luhn_obj.input_number if luhn_obj.valid?
      counter += 1 
    end 
  end
  ...
  def addends
    array_numbers = @input_number.to_s.chars.map(&:to_i)
    luhn_array = []
    array_numbers.reverse.each_with_index do |num, idx|
      luhn_array << (num * 2) if idx.odd? && (num * 2) < 10 
      luhn_array << ((num * 2) - 9) if idx.odd? && (num * 2) >= 10
      luhn_array << num if idx.even? || idx.zero?
    end
    luhn_array.reverse
  end
